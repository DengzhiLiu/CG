...
* $Id: vector.h
*
* This file implement a mathlib of vector
* 3D vector is a useful data type in computer graphics
*
...
//operators
	Real* operator&() { return m; }
    inline Real operator*(const Self &other) const { return accumulate(multiplies<Real>(), plus<Real>(), other); }
    inline Self operator+(const Self &other) const { return apply(plus<Real>(), other); }
    inline Self operator-(const Self &other) const { return apply(minus<Real>(), other); }
    inline Self operator*(const Real &scalar) const { return apply(bind2nd(multiplies<Real>(), scalar)); }
    inline Self operator/(const Real &scalar) const { return apply(bind2nd(divides<Real>(), scalar)); }
    inline Self operator-() const { return apply(negate<Real>()); }
    inline bool operator==(const Self &other) const { return accumulate(equal_to<Real>(), logical_and<Real>(), other); }
...
public:
    ...
    inline Real &operator[](int n) { if((int)m.size() <= n) m.resize(n + 1); return m[n]; }
    inline const Real &operator[](int n) const { if((int)m.size() <= n) const_cast<Vector<Real, -1> *>(this)->m.resize(n + 1); return m[n]; }
...
template <class Real, int Dim>
inline Vector<Real, Dim> operator*(const Real &scalar, const Vector<Real, Dim> &vec)
{
...
//cross product
template <class Real>
inline Vector<Real, 3> operator%(const Vector<Real, 3> &v1, const Vector<Real, 3> &v2)
{
