...
public:
	...
	inline explicit Vector3( const float afCoordinate[3] )
		...
		*/
		...
		//	Quaternion q;
		//	q.fromAxisAngle( rand()/0x7fff *2*PI*180, *this->x,*this->y,*this->z );
		//	newUp = q * newUp;

		//	// Finally rotate this by given angle around randomised up
		//	q.fromAxisAngle( angle, newUp.x;newUp.y,newUp.z );
		//	return q * (*this);
		//}
			...
			const Vector3& fallbackAxis = Vector3::ZERO) const
		{
			...
			if (d < (1e-6f - 1.0f))
			{
				if (fallbackAxis != Vector3::ZERO)
				{
					...
					q.fromAxisAngle(PI, fallbackAxis.x,fallbackAxis.y,fallbackAxis.z);
				}
				else
				{
					// Generate an axis
					...
					axis.normalise();
					q.fromAxisAngle(PI, axis.x,axis.y,axis.z);
				}
			}
			else
			{
		...
		*/
		...
		inline void interpolation(const Vector3 &A,const Vector3 &B,float alpha )
		{
			Vector3 temp =alpha*A+(1-alpha)*B;
			x=temp.x;
			y=temp.y;
			z=temp.z;
		}
