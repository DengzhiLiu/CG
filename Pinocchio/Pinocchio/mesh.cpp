...
void Mesh::computeVertexNormals()
{
    ...
    for(i = 0; i < (int)edges.size(); i += 3) {
		...
		Vector3 normal = ((vertices[i1].pos - vertices[i2].pos) % (vertices[i2].pos - vertices[i3].pos)).normalize();
    ...
    while(!strm.eof()) {
        ...
        if(words[0][0] == 'f') {
			...
			if ((int)words.size() == 4)	{
                ...
                edges[first + 1].vertex = a[1] - 1;
                edges[first + 2].vertex = a[2] - 1;
            }
			else if ((int)words.size() == 5) {
				int first = edges.size();
				edges.resize(edges.size() + 6);
				edges[first].vertex = a[0] - 1;
				edges[first + 1].vertex = a[1] - 1;
				edges[first + 2].vertex = a[2] - 1;
				edges[first + 3].vertex = a[2] - 1;
				edges[first + 4].vertex = a[3] - 1;
				edges[first + 5].vertex = a[0] - 1;
			}
...
bool Mesh::isConnected() const
{
    ...
    int inTodo = 0;
	// 如果模型中存在孤立的体素怎么办，就是不与任何大的模型相连的小的模型
