...
public:
	...
	// 这段程序的作用是？？？
	/* 将生成的八叉树结构root存储在线性查找表table中，table的长度是1<< (16 - (16%Dim))
		分成了三维，每一维的长度是bit/Dim												*/
    void preprocessIndex()
    {
        for(int i = 0; i < (1 << bits); ++i) {
            ...
            while(table[i]->getChild(0) && cnt < (bits / Dim)) {
				...
				// 把cur(0~1<<bits)分成bits/Dim块，每块中cur右移Dim个长度，然后每次循环bits/Dim次
                cur = cur >> Dim;
            }
        }
    }

	// distanceField->locate(vecs[i])
    Node *locate(const Vec &v) const
    {
        unsigned int idx = _lookup(v);
